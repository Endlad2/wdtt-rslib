...
static ACTIVE_FINGERPRINT: LazyLock<RwLock<String>> = LazyLock::new(|| RwLock::new("chrome".into()));
pub fn SetActiveFingerprint(fp: &str) { *ACTIVE_FINGERPRINT.write().unwrap() = fp.into(); }
pub fn GetActiveFingerprint() -> String { ACTIVE_FINGERPRINT.read().unwrap().clone() }
